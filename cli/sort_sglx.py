...
def run_ks2_helper(ap_fn):
    ...
    if platform.system()=='Linux':
        ks2_matlab_dir = '/active/ramirez_j/ramirezlab/nbush/helpers/kilosort2'
    if platform.system()=='Windows':
        ks2_matlab_dir = r'Y:\helpers\kilosort2'
    bin_dir,basename = os.path.split(ap_fn)
    ...
    output_dir = os.path.split(ap_fn)[0]

    mask = get_noise_channels(ap_fn,
                              meta,
                              num_channels,
                              sample_rate,
                              bit_volts)
    ...
    MetaToCoords(metaFullPath=Path(meta_fn), outType=1, badChan=MaskChannels, destFullPath=output_dir)


    ephys_params=get_ephys_params(meta)
    ks2_params = Kilosort2Parameters().dump(0)
    ks2_params['fproc'] = f"fullfile('{bin_dir}','temp_wh.dat'"
    ks2_params['fproc'].replace('\\','//')
    ks2_params['chanMap'] ="'" +  bin_dir.split('\\')[-1]+'.mat'+"'"
    ks2_output_dir = os.path.join(bin_dir,'ks2')
    ks2_params['minFr'] = 0
    # =============== #
    # Generate Kilosort Config file
    # ==============#
    matlab_file_generator.create_config2(ks2_matlab_dir,
                                         ks2_output_dir,
                                         os.path.split(ap_fn)[0],
                                         ephys_params,
                                         ks2_params)
